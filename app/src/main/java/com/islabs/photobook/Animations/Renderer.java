...
public class Renderer implements GLSurfaceView.Renderer {
	...
	private Vector<PageMesh> mPageMeshes;
	 ...
	 * Basic constructor.
	 */
	public Renderer(Observer observer) {
		mObserver = observer;
		mPageMeshes = new Vector<PageMesh>();
		mPageRectLeft = new RectF();
		mPageRectRight = new RectF();
	}

	/**
	 * Adds PageMesh to this renderer.
	 */
	public synchronized void addCurlMesh(PageMesh mesh) {
		removeCurlMesh(mesh);
		mPageMeshes.add(mesh);
	}
	...
	public synchronized void onDrawFrame(GL10 gl) {
		...
		for (int i = 0; i < mPageMeshes.size(); ++i) {
			mPageMeshes.get(i).onDrawFrame(gl);
		}
	}
	...
	/**
	 * Removes PageMesh from this renderer.
	 */
	public synchronized void removeCurlMesh(PageMesh mesh) {
		while (mPageMeshes.remove(mesh))
			;
	}
