...
package com.islabs.photobook.Animations;
...
public class AlbumView extends GLSurfaceView implements View.OnTouchListener,
    ...
    // show page flipping.
    private PageMesh mPageCurl;

    private PageMesh mPageLeft;
    private PageProvider mPageProvider;
    private PageMesh mPageRight;

    private PointerPosition mPointerPos = new PointerPosition();

    private com.islabs.photobook.Animations.Renderer mRenderer;
    ...
    private void init(Context ctx) {
        mRenderer = new com.islabs.photobook.Animations.Renderer(this);
        ...
        mPageLeft = new PageMesh(10);
        mPageRight = new PageMesh(10);
        mPageCurl = new PageMesh(10);
    ...
    public void onDrawFrame() {
        ...
        if (currentTime >= mAnimationStartTime + mAnimationDurationTime) {
            if (mAnimationTargetEvent == SET_CURL_TO_RIGHT) {
                ...
                PageMesh right = mPageCurl;
                PageMesh curl = mPageRight;
                right.setRect(mRenderer.getPageRect(com.islabs.photobook.Animations.Renderer.PAGE_RIGHT));
                ...
                PageMesh left = mPageCurl;
                PageMesh curl = mPageLeft;
                left.setRect(mRenderer.getPageRect(com.islabs.photobook.Animations.Renderer.PAGE_LEFT));
    ...
    public boolean onTouch(View view, MotionEvent me) {
        ...
        RectF rightRect = mRenderer.getPageRect(com.islabs.photobook.Animations.Renderer.PAGE_RIGHT);
        RectF leftRect = mRenderer.getPageRect(com.islabs.photobook.Animations.Renderer.PAGE_LEFT);
                ...
                if (mCurlState == CURL_LEFT || mCurlState == CURL_RIGHT) {
                    ...
                    if ((mViewMode == SHOW_ONE_PAGE && mPointerPos.mPos.x > (rightRect.left + rightRect.right) / 2)
                        ...
                        mAnimationTarget.x = mRenderer
                                .getPageRect(com.islabs.photobook.Animations.Renderer.PAGE_RIGHT).right;
    ...
    private void setCurlPos(PointF curlPos, PointF curlDir, double radius) {
        ...
        if (mCurlState == CURL_RIGHT
            ...
            RectF pageRect = mRenderer.getPageRect(com.islabs.photobook.Animations.Renderer.PAGE_RIGHT);
            ...
            RectF pageRect = mRenderer.getPageRect(com.islabs.photobook.Animations.Renderer.PAGE_LEFT);
            if (curlPos.x <= pageRect.left) {
                mPageCurl.reset();
                requestRender();
                return;
    ...
    public void setViewMode(int viewMode) {
        switch (viewMode) {
            case SHOW_ONE_PAGE:
                mViewMode = viewMode;
                mPageLeft.setFlipTexture(true);
                mRenderer.setViewMode(com.islabs.photobook.Animations.Renderer.SHOW_ONE_PAGE);
                break;
            case SHOW_TWO_PAGES:
                mViewMode = viewMode;
                mPageLeft.setFlipTexture(false);
                mRenderer.setViewMode(com.islabs.photobook.Animations.Renderer.SHOW_TWO_PAGES);
                break;
        }
    }
        ...
        switch (page) {
            ...
            case CURL_RIGHT: {
                ...
                if (mCurrentIndex > 0) {
                    mPageLeft.setFlipTexture(true);
                    mPageLeft
                            .setRect(mRenderer.getPageRect(com.islabs.photobook.Animations.Renderer.PAGE_LEFT));
                    mPageLeft.reset();
                    if (mRenderLeftPage) {
                ...
                if (mCurrentIndex < mPageProvider.getPageCount() - 1) {
                    ...
                    mPageRight.setRect(mRenderer
                            .getPageRect(com.islabs.photobook.Animations.Renderer.PAGE_RIGHT));
                    mPageRight.setFlipTexture(false);
                    mPageRight.reset();
                ...
                mPageCurl.setRect(mRenderer.getPageRect(com.islabs.photobook.Animations.Renderer.PAGE_RIGHT));
    ...
    private void updateCurlPos(PointerPosition pointerPos) {
        ...
        if (mCurlState == CURL_RIGHT
            ...
            float pageWidth = mRenderer.getPageRect(com.islabs.photobook.Animations.Renderer.PAGE_RIGHT)
                    .width();
            ...
            if (dist >= curlLen) {
                ...
                if (mViewMode == SHOW_TWO_PAGES) {
                    ...
                    float pageLeftX = mRenderer
                            .getPageRect(com.islabs.photobook.Animations.Renderer.PAGE_RIGHT).left;
        ...
        // Otherwise we'll let curl follow pointer position.
            ...
            float pageLeftX = mRenderer.getPageRect(com.islabs.photobook.Animations.Renderer.PAGE_RIGHT).left;
            radius = Math.max(Math.min(mCurlPos.x - pageLeftX, radius), 0f);

            float pageRightX = mRenderer.getPageRect(com.islabs.photobook.Animations.Renderer.PAGE_RIGHT).right;
    ...
    /**
     * Updates given AlbumPage via PageProvider for page located at index.
     */
    private void updatePage(AlbumPage page, int index) {
    ...
    private void updatePages() {
        ...
        if (rightIdx >= 0 && rightIdx < mPageProvider.getPageCount()) {
            ...
            mPageRight.setFlipTexture(false);
            mPageRight.setRect(mRenderer.getPageRect(com.islabs.photobook.Animations.Renderer.PAGE_RIGHT));
            mPageRight.reset();
            mRenderer.addCurlMesh(mPageRight);
        }
        if (leftIdx >= 0 && leftIdx < mPageProvider.getPageCount()) {
            ...
            mPageLeft.setFlipTexture(true);
            mPageLeft.setRect(mRenderer.getPageRect(com.islabs.photobook.Animations.Renderer.PAGE_LEFT));
            mPageLeft.reset();
            if (mRenderLeftPage) {
                mRenderer.addCurlMesh(mPageLeft);
            }
    ...
    public interface PageProvider {
         ...
         * of any size though. You should use provided AlbumPage for storing page
         * information for requested page number.<br/>
         * <br/>
         * Index is a number between 0 and getBitmapCount() - 1.
         */
        public void updatePage(AlbumPage page, int width, int height, int index);
    }
