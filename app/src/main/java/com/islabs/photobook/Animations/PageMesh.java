...
public class PageMesh {
	...
	private final AlbumPage mTexturePage = new AlbumPage();
	 ...
	 */
	public PageMesh(int maxCurlSplits) {
		...
		for (int i = 0; i < mArrScanLines.size(); ++i) {
			...
			while (mArrOutputVertices.size() > 0) {
				...
				if (textureFront != mFlipTexture) {
					...
					v.mColor = mTexturePage.getColor(AlbumPage.SIDE_FRONT);
				} else {
					v.mTexX *= mTextureRectBack.right;
					v.mTexY *= mTextureRectBack.bottom;
					v.mColor = mTexturePage.getColor(AlbumPage.SIDE_BACK);
				}
	 ...
	 */
	public synchronized AlbumPage getTexturePage() {
		return mTexturePage;
	}
	...
	public synchronized void reset() {
		...
		for (int i = 0; i < 4; ++i) {
			...
			if (mFlipTexture) {
				...
				tmp.mColor = mTexturePage.getColor(AlbumPage.SIDE_BACK);
			} else {
				tmp.mTexX *= mTextureRectFront.right;
				tmp.mTexY *= mTextureRectFront.bottom;
				tmp.mColor = mTexturePage.getColor(AlbumPage.SIDE_FRONT);
			}
